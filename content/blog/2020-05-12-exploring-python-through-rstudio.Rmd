...
Another similar function of languages is plotting. In R, "ggplot2" can be fairly equivicated to "matplotlib" in Python. Below, we take a look at ggplot2 being put to use:
```{R}
#R chunk:
...
All of these R/Python interactions can be especially helpul in more complex situations where one language might be more beneficial than the other. For someone like me who prefers the R-studio interphase over Jupyter notebook, I can utilize these aspects to jump back and forth between R and Python without ever leaving R-studio. 